/*
 * File: Class file to make our T-Shaped piece.
 * 
 * CS 342, Spring 2016
 * Project 5: Tetris
 * 
...
public class TBlock implements Tetromino {
 ...
 private Block one = new Block(Color.MAGENTA);
 private Block two = new Block(Color.MAGENTA);
 private Block three = new Block(Color.MAGENTA);
 private Block four = new Block(Color.MAGENTA);

 public TBlock(Block[][] grid) {
  grid[3][0] = one;
  grid[4][0] = two;
  grid[4][0] = three;
  grid[5][0] = four;
 ...
 public void rotate(Block[][] grid) {
  ...
  if (rotation == 0) {
   int[][] arr = {
     ...
     {pThree[0]+1,pThree[1]+1}, {pFour[0]-1,pFour[1]+1},
   };
   if (!checkBlocks(grid, arr)) {
    return;
   }
