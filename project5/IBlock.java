/*
 * File: Class file to make our I-Shaped piece.
 * 
 * CS 342, Spring 2016
 * Project 5: Tetris
 * 
...
public class IBlock implements Tetromino {
 ...
 private Block two = new Block(Color.CYAN);
 private Block three = new Block(Color.CYAN);
 private Block four = new Block(Color.CYAN);

 public IBlock(Block[][] grid) {
  grid[3][0] = one;
  grid[4][0] = two;
  grid[5][0] = three;
  grid[6][0] = four;
 ...
 public void rotate(Block[][] grid) {
  ...
  if (rotation == 0) {
   int[][] arr = {
     {pOne[0]+2,pOne[1]-1}, {pTwo[0]+1,pTwo[1]}, 
     {pThree[0],pThree[1]+1}, {pFour[0]-1,pFour[1]+2},
   };
   if (!checkBlocks(grid, arr)) {
    return;
   }
   ...
   grid[pOne[0]+2][pOne[1]-1] = one;
   grid[pTwo[0]+1][pTwo[1]] = two;
   grid[pThree[0]][pThree[1]+1] = three;
   grid[pFour[0]-1][pFour[1]+2] = four;
   ...
   int[][] arr = {
     {pOne[0]+1,pOne[1]+2}, {pTwo[0],pTwo[1]+1}, 
     {pThree[0]-1,pThree[1]}, {pFour[0]-2,pFour[1]-1},
   };
   if (!checkBlocks(grid, arr)) {
    return;
   }
   ...
   grid[pOne[0]+1][pOne[1]+2] = one;
   grid[pTwo[0]][pTwo[1]+1] = two;
   grid[pThree[0]-1][pThree[1]] = three;
   grid[pFour[0]-2][pFour[1]-1] = four;
